...
class RNNDecoderSep(nn.Module):
    def __init__(self, max_step, vocab: Vocabulary, dim_img, dim_ppl, dim_word, hidden_size, dropout, device,
        ...
        self.attn_ppl_1 = Attention(query_size=hidden_size, memory_size=dim_ppl, hidden_size=hidden_size,
                                  has_bias=True, dropout=dropout)
        self.attn_ppl_2 = Attention(query_size=hidden_size, memory_size=dim_ppl, hidden_size=hidden_size,
                                  has_bias=True, dropout=dropout)
    ...
    def _decode_step(self, dec_input_emb, rnn_state, image_feats, image_pool, ppl_feats, answer_hint, ppl_mask=None):
        ...
        ppl_attn, _ = self.attn_ppl_1(query=h_vis, memory=ppl_feats, memory_mask=ppl_mask)
        ppl_attn_1, _ = self.attn_ppl_2(query=h_vis, memory=ppl_feats, memory_mask=1 - ppl_mask)
        if self.fuse_strategy == "average":
            language_lstm_input = torch.cat((image_attn + ppl_attn + ppl_attn_1, h_vis), dim=-1)
        elif self.fuse_strategy == "concat":
