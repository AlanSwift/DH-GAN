...
class Discriminator(nn.Module):
    def __init__(self, vocab, d_v, d_word, d_model, dropout=0.2):
        ...
        self.vis_emb = nn.Sequential(nn.Linear(self.d_model + 1 * d_word, self.d_model),
                                        nn.LeakyReLU(0.1),
                                        nn.Dropout())
        self.pred = nn.Sequential(nn.Linear(self.d_model + d_word, self.d_model),
                                nn.Tanh(),
                                nn.Dropout(dropout),
    ...
    def forward(self, visual_feats, visual_spatial_feats, visual_hints, question, question_mask, answer, labels=None):
        ...
        a_feats, a_vec, a_mask = self.ans_encoder(answer)
        ...
        # paded_neg = pad_sequence(neg_collect).transpose(0, 1)
        # query = torch.cat((q_vec, a_vec), dim=1)

        attn_res, _ = self.attn(query=q_vec, memory=paded_pos, memory_mask=attn_mask)
        
        # attn_neg, _ = self.neg_attn(query=query, memory=paded_neg)
        

        # q_agg = self.interaction_v_q(query=v_feats, memory=q_feats, memory_mask=q_mask)

        # # a_feats, _, a_mask = self.text_encoder(answer)
        # # a_feats = a_feats.transpose(0, 1)
        # # a_agg = self.interaction_v_a(query=v_feats, memory=a_feats, memory_mask=a_mask)

        # feats_fused = torch.cat((v_feats, q_agg), dim=-1)
        # v_emb = self.vis_emb(feats_fused)
        # v_emb = self.obj_interact(v_emb)
        # v_pool = v_emb.mean(1)
        ppl_logits = self.pred(torch.cat((attn_res, a_vec), dim=-1)).squeeze(1)
